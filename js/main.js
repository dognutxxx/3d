...
const material = new THREE.ShaderMaterial({
  uniforms: {
    uMix: { value: 0 },
    uColorA: { value: new THREE.Color(0xffdd44) },
...
previewBox.style.cssText = 'position:fixed;top:24px;right:24px;width:200px;max-height:380px;background:rgba(0,0,0,0.6);border-radius:12px;border:1px solid rgba(255,255,255,0.2);z-index:10;overflow:hidden;display:none;box-shadow:0 4px 20px rgba(0,0,0,0.4);';
...
sizeRow.style.cssText = 'padding:8px 12px;border-bottom:1px solid rgba(255,255,255,0.1);display:flex;align-items:center;gap:6px;flex-wrap:wrap;font-size:11px;color:rgba(255,255,255,0.85);font-family:system-ui;';
const labelW = document.createElement('span');
labelW.textContent = 'กว้าง';
const inputW = document.createElement('input');
inputW.type = 'number';
inputW.min = 80;
inputW.max = 600;
inputW.value = 200;
inputW.style.cssText = 'width:56px;padding:4px 6px;background:rgba(0,0,0,0.4);border:1px solid rgba(255,255,255,0.25);border-radius:6px;color:#fff;font-size:12px;';
const labelH = document.createElement('span');
labelH.textContent = 'ยาว';
const inputH = document.createElement('input');
inputH.type = 'number';
inputH.min = 80;
inputH.max = 500;
inputH.value = 280;
inputH.style.cssText = 'width:56px;padding:4px 6px;background:rgba(0,0,0,0.4);border:1px solid rgba(255,255,255,0.25);border-radius:6px;color:#fff;font-size:12px;';
sizeRow.appendChild(labelW);
sizeRow.appendChild(inputW);
sizeRow.appendChild(document.createTextNode(' × '));
sizeRow.appendChild(labelH);
sizeRow.appendChild(inputH);
function applyPreviewSize() {
  const w = Math.max(80, Math.min(600, Number(inputW.value) || 200));
  const h = Math.max(80, Math.min(500, Number(inputH.value) || 280));
  previewBox.style.width = w + 'px';
  previewBox.style.maxHeight = h + 'px';
  previewImg.style.maxHeight = Math.max(100, h - 100) + 'px';
}
inputW.addEventListener('change', applyPreviewSize);
inputH.addEventListener('change', applyPreviewSize);
inputW.addEventListener('input', applyPreviewSize);
inputH.addEventListener('input', applyPreviewSize);
previewBox.appendChild(previewLabel);
previewBox.appendChild(sizeRow);
...
// ========== หมุนด้วยเมาส์ (ปิดการใช้งาน — ล็อคทั้งหน้าไม่ให้หมุน) ==========
